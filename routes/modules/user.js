...
router.post('/register', wrapAsync(verifyRegisterData), wrapAsync(User.registerUser))

router.get('/login', wrapAsync(User.renderLoginPage))
router.post('/login', passport.authenticate('local', { failureRedirect: '/user/login' }), wrapAsync(User.login))
router.get('/logout', authenticated, wrapAsync(User.logout))

router.get('/profile', authenticated, wrapAsync(User.renderProfilePage))
router.delete('/deregister', authenticated, wrapAsync(Studio.deregisterCourse))
